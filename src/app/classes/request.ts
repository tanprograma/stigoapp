...
export class Request {
  ...
  public getPayload() {
    return {
      host: this.host.toLowerCase(),
      destination: this.destination,
      commodities: this.commodities,
      isIssued: this.isIssued,
    };
  }
  public validate(hosts: any, commodities: any) {
    const isHost = this.validateHost(hosts);
  ...
  private validateHost(hosts: any) {
    return hosts.includes(this.host.toLowerCase());
  }
